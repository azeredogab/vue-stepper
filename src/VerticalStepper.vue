          ...
          <div class="side">
              <div class="steps-wrapper">
                  ...
                  <template v-for="(step, index) in steps">
                      <div :class="['step', isStepActive(index, step)]" :key="index" :style="{backgroundColor: step.color || '#276bce' }">
                          <div class="step-title">
                              <h4><span v-if="numeral">{{index + 1}}.</span> {{step.title}}</h4>
                          </div>
                      </div>
                  </template>
  ...
  computed: {
    enterAnimation() {
      if (this.currentStep.index < this.previousStep.index) {
        return "animated faster fadeInLeft";
      } else {
        return "animated faster fadeInRight";
      }
    },
    leaveAnimation() {
      if (this.currentStep.index > this.previousStep.index) {
        return "animated faster fadeOutLeft";
      } else {
        return "animated faster fadeOutRight";
      }
    }
  },
  ...
  methods: {
    ...
    backStep() {
      ...
      if (currentIndex >= 0) {
        ...
        if (this.steps[currentIndex].completed)
          this.canContinue = true;
...
<style src="./VerticalStepper.scss" scoped lang="scss">
